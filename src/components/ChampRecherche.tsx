...
export function ChampRecherche({ className = '' }: { className?: string }) {
  return (
    <div className={`relative -ml-2 -mb-7 pb-7 pl-5 pr-7 pt-3.5 ${className}`}>
      {/*
       * Le contour partiel : haut, biseau, droite. Pas de trait à gauche ni
       * en bas — ceux-là appartiennent au cadre de l'accroche, et les
       * redessiner ici produirait un double trait à un pixel d'écart.
       */}
      <span className="pointer-events-none absolute left-[17px] right-0 top-0 h-px bg-white/[0.16]" aria-hidden />
      <span
        className="pointer-events-none absolute left-0 top-[17px] h-px w-[24px] origin-left -rotate-45 bg-white/[0.16]"
        aria-hidden
      />
      <span className="pointer-events-none absolute bottom-0 right-0 top-0 w-px bg-white/[0.16]" aria-hidden />
